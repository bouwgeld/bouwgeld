...
#if defined(NDEBUG)
# error "BouwGeld cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x96a85099cbc37a7bf586a45d1833c9005ece630766d6d83629880bab20516eb0");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // BouwGeld: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "BouwGeld Signed Message:\n";
...
{
    // BouwGeld: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // BouwGeld: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // BouwGeld: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1532937333, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "200718 setted up by teslavr";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("041c98a6dc697349fd80cd58334f04347f9467249bffaa8e865957390812e3b0c348341fec26078f281b93a16369033c30cc43de71b80e4131fe44d798862bdff8") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1532937333;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1568413;
        ...
        assert(block.hashMerkleRoot == uint256("0xb128f0fd8b7599a3f17e533ea5d7705ef736e4b1528298eb4095618d38df585d"));
...
unsigned char pchMessageStart[4] = { 0x42, 0xee, 0x09, 0x2a }; // BouwGeld: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BouwGeldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BouwGeldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BouwGeldMiner(CWallet *pwallet)
{
    printf("BouwGeldMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bouwgeld-miner");
    ...
    try { loop {
        ...
        printf("Running BouwGeldMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BouwGeldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BouwGeldMiner, pwallet));
}
